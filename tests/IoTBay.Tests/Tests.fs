...
let createHost () =
    WebHostBuilder()
        ...
        .Configure(Action<IApplicationBuilder> IoTBay.App.configureApp)
        .ConfigureServices(Action<IServiceCollection> IoTBay.App.configureServices)
...
let ``Route / returns "Hello world, from Giraffe!"`` () =
    ...
    |> ensureSuccess
    |> readText
    |> shouldContain "IoTBay Online Store"
